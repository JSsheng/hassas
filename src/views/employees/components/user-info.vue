<template>
  <div class="user-info">
    <i class="el-icon-printer" @click="$router.push('/employees/print/ ' + '?type=personal')" />
    <!-- 个人信息 -->
...
import { getEmployessInfo, saveEmployeesInfo } from '@/api/employees.js'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async loadEmployes() {
      const res = await getEmployessInfo(this.userId)
      this.formData = res
    },
