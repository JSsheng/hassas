<template>
  <div class="job-info">
    <!-- 基础信息 -->
    <i class="el-icon-printer" @click="$router.push('/employees/print/ ' + '?type=job')" />
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="saveEmplayeesJobs">保存更新</el-button>
...
import { getEmployeesJosInfo, saveEmployeesJosInfo } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    async loadEmployeesJob() {
      this.formData = await getEmployeesJosInfo(this.userId)
    },
    async saveEmplayeesJobs() {
      await saveEmployeesJosInfo(this.formData)
